...
KEY_DRY_OFF = 'Off'
KEY_DRY_DISCONNECTED = 'Disconnected'
...
class LGDryerDevice(LGDevice):
    ...
    def remaining_time_in_minutes(self):
        # The API (indefinitely) returns 1 minute remaining when a cycle is
        # either in state off or complete, or process drying. Return 0
        # minutes remaining in these instances, which is more reflective of
        # reality.
        if (self._status and
            (self._status.state == dryer.DryerState.OFF or
             self._status.state == dryer.DryerState.END)):
            return 0
        return self._status.remaining_time if self._status else 0
    ...
    def course(self):
        if self._status:
            if self._status.smart_course != KEY_DRY_OFF:
                return self._status.smart_course
            else:
                return self._status.course
        return KEY_DRY_OFF
